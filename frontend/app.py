...
from ai_engine.batch_runner import COLUMN_MAP   # ← add with the other imports
...
from ai_engine.batch_runner import row_to_user          # mapping
...
app = Flask(__name__)
app.secret_key = "diet-plan-secret"           # for flash messages

# basic bootstrap-ish styling for quick demo
STYLE = """
...
<thead class="table-light"><tr>
 <th>Name</th><th>Status</th><th>Time&nbsp;(s)</th>
</tr></thead><tbody>
...
def upload():
    ...
    save_path = UPLOAD_FOLDER / f"upload_{ts}.csv"
    file.save(save_path)

    df = pd.read_csv(save_path)
    if df.empty:
        flash("CSV appears to have no rows")
        return redirect(url_for("index"))

    return render_template_string(
        SELECT_TPL, df=df, csv_path=str(save_path)
    )
...
def generate():
    ...
    if "ALL" in selected or not selected:
        ...
        df = pd.read_csv(csv_path).loc[[int(i) for i in selected]]
        
    df = df.rename(columns=COLUMN_MAP)              # ← NEW: guarantees “Name”
    ...
    for idx, row in df.iterrows():
        ...
        name_raw  = user["user_profile"]["Name"] or f"row_{idx}"
        safe_name = re.sub(r"[^A-Za-z0-9_\-]+", "_", name_raw.strip()) or f"row_{idx}"
    ...
    zip_name = out_dir.with_suffix(".zip").name
    ...
    return render_template_string(
        RESULT_TPL, results=results,
        zip_ready=True, zip_name=zip_name
    )
