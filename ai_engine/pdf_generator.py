...
def create_pdf(plan: dict, filename: str):
    ...
    # Sort meal keys in proper sequence: Breakfast, Snacks, Lunch, Snacks, Dinner
    def get_meal_order(meal_name):
        ...
        if "breakfast" in meal_lower:
            ...
            return 4  # Other meals go between lunch and dinner
    
    meal_keys = sorted(meal_keys, key=get_meal_order)  # e.g., ['Breakfast', 'Snacks', 'Lunch', 'Dinner']
    ...
    for entry in plan.get("7DayPlan", []):
        cells = [str(entry.get("Day", ""))] + [str(entry.get(key, "")) for key in meal_keys]
